...
labevents_pq <- open_dataset(input_file, format = "csv")
output_dir <- "labevents_parquet"
write_dataset(labevents_pq, path = output_dir, format = "parquet")
```

```{r}
labevents_pq <- open_dataset("labevents_parquet", format = "parquet")
```
...
transfers <- transfers %>%
  mutate(
    ...
    unit_type = case_when(
      careunit %in% c("MICU", "SICU", "CCU", "TSICU") ~ "ICU",
      TRUE ~ "General"
    )
  )
```
...
```{r}
ggplot() +
  # ADT Events as segments (Fix size → linewidth)
  geom_segment(data = transfers,
               aes(x = intime, xend = outtime, y = "ADT", yend = "ADT", color = careunit),
               linewidth = 3) +  # 🔧 Fix deprecated size aesthetic

  # Lab Events as cross markers (Ensure data exists)
  geom_point(data = labevents,
             aes(x = charttime, y = "Lab"),
             shape = 3, size = 3) +

  # Procedures as different shapes (Ensure data exists)
  geom_point(data = procedures,
             aes(x = charttime, y = "Procedure", shape = long_title),
             size = 4) +

  # Patient Information
  ggtitle(paste("Patient", patients$subject_id, ",",
                patients$gender, ",",
                patients$anchor_age, "years old,", patients$race),
          subtitle = paste(diagnoses$long_title, collapse = "\n")) +

  # Use Correct Color Mapping
  scale_color_manual(values = care_unit_colors) +
  
  # Use Different Shapes for Procedures

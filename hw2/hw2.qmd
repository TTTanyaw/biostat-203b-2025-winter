...
# Run time
system.time(df_tidy_appro <- read_csv(file_path_admis, col_types = appro_data_type))
...
```{r, eval=FALSE}
system.time(df_selected <- read_csv(file_path_lab, col_select = c("subject_id", "itemid", "charttime", "valuenum")))
```
...
# select columns and filter `itemid`
...
```
The ingest + select + filter process takes less than 1 minute.
